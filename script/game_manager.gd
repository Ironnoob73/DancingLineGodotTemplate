...
func _process(delta: float) -> void:
	...
	if state == GameState.PLAYING or state == GameState.READY:
		print("State: %s, Time: %.2f/%.2f, Progress: %d%%, Diamonds: %d/%d, Crowns: %d/%d" % [state, elapsed_time, music_length, roundi(game_percentage * 100), diamond_count, max_diamonds, crown_count, max_crowns])
